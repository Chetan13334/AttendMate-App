  ...
  if (loading) {
    return (
      <div style={{ backgroundColor: '#ffffff', padding: '0 16px', marginTop: '16px', marginBottom: '100px' }}>
        {/* Skeleton for birthday card */}
        <div style={{ width: '96%', margin: '0 auto 20px' }}>
          <Skeleton style={{ height: '5rem', borderRadius: '1rem' }} />
        </div>
        
        {/* Skeleton for "TODAY'S EVENTS" title */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {[...Array(4)].map((_, index) => (
            <Skeleton key={index} style={{ height: '4rem', borderRadius: '1rem' }} />
          ))}
        </div>
